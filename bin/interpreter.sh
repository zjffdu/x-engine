...
ZEPPELIN_INTERPRETER_API_JAR=$(find "${ZEPPELIN_HOME}/interpreter" -name 'xengine-interpreter-api-*.jar')
ZEPPELIN_INTP_CLASSPATH="${CLASSPATH}:${ZEPPELIN_INTERPRETER_API_JAR}"

# construct classpath
if [[ -d "${ZEPPELIN_HOME}/xengine-interpreter/target/classes" ]]; then
  ZEPPELIN_INTP_CLASSPATH+=":${ZEPPELIN_HOME}/xengine-interpreter/target/classes"
fi

# add test classes for unittest
if [[ -d "${ZEPPELIN_HOME}/xengine-core/target/test-classes" ]]; then
  ZEPPELIN_INTP_CLASSPATH+=":${ZEPPELIN_HOME}/xengine-core/target/test-classes"
  addJarInDirForIntp "${ZEPPELIN_HOME}/xengine-core/target/test-classes"
fi

addJarInDirForIntp "${ZEPPELIN_HOME}/xengine-interpreter-api/target"
...
ZEPPELIN_PID="${ZEPPELIN_PID_DIR}/xengine-interpreter-${INTERPRETER_ID}-${ZEPPELIN_IDENT_STRING}-${HOSTNAME}-${PORT}.pid"
ZEPPELIN_LOGFILE="${ZEPPELIN_LOG_DIR}/xengine-interpreter-${INTERPRETER_GROUP_ID}-"
